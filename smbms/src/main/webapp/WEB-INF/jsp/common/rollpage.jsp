...
<body>
        ...
        <div style="float: right">跳转至
        	<input type="text" name="inputPage" id="inputPage" style="width: 20px" /> 页
        	<button type="button" onClick='jump_to(document.forms[0],document.getElementById("inputPage").value)'>GO</button>&nbsp;&nbsp;&nbsp;&nbsp;
        	
        	<c:if test="${param.currentPageNo > 1}">
				<button onclick="javascript:page_nav(document.forms[0],1);">首页</button>
				<button type="button" onclick="javascript:page_nav(document.forms[0],${param.currentPageNo-1});">&laquo;上一页</button>
			</c:if>
			<c:if test="${param.currentPageNo < param.totalPageCount }">
				<button type="button" onclick="javascript:page_nav(document.forms[0],${param.currentPageNo+1 });">下一页&raquo;</button>
				<button onclick="javascript:page_nav(document.forms[0],${param.totalPageCount });">最后一页</button>
			</c:if>
        </div>
